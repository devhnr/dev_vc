...
class MyaccountController extends Controller
{
 ...
 public function order_details($id) {
    ...
    $query = DB::table('ci_orders')
        ->leftJoin('frontloginregisters', 'ci_orders.user_id', '=', 'frontloginregisters.id')
        ->leftJoin('ci_shipping_address', 'ci_orders.order_id', '=', 'ci_shipping_address.order_id')
        ->select('frontloginregisters.email as user_email', 
                 'frontloginregisters.name as user_name', 
                 'frontloginregisters.mobile as user_mobile',  
                 'ci_orders.*',  
                 'ci_shipping_address.*')
        ->where('ci_orders.user_id', $userid)
        ->where('ci_orders.order_id', $id)  // Use $id directly for the specific order
        ->first();  // Get a single order
    ...
    foreach ($itemList as $item) {
        ...
        // Calculate the product item price considering discount
    ...
    // Attach the items and total to the order object
    $query->items = $itemList;
    $query->sub_total = $total;

    // Prepare data for the view
    $data['orders'] = $query;  // Change to a single order instead of a list
